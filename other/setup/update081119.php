...
if(!file_exists($lfile)){
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		$query=sprintf("alter table %s add avator int(5) NOT NULL default '10'", $dbprefix.'main');
		$result=mysql_query($query);
		echo '<li>更新数据表 '.$dbprefix.'main：<span style="font-weight:bold;color:#'.($result==true?'036;">成功':'f00;">失败').'</span></li>';
		unset($query);
		unset($result);

		$query=sprintf("alter table %s change photo photo text", $dbprefix.'member');
		$result=mysql_query($query);
...
?>
	<div class="title">从 1.0.6 升级到 1.0.7</div>
	<div class="lcontent">
		<form method="post">
			此升级程序只能从 <strong>1.0.6</strong> 升级到 1.0.7，版本太低的用户请先升级到 1.0.6，再使用此升级程序！<br/><br/>
